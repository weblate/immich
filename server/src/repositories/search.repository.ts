...
export class SearchRepository implements ISearchRepository {
  ...
  searchDuplicates(): Promise<AssetDuplicateResult[]> {
    return Promise.resolve([] as unknown as AssetDuplicateResult[]);
  ...
  private searchAssetBuilder(options: AssetSearchBuilderOptions, kysely: Kysely<DB> = this.prismaRepository.$kysely) {
    ...
    const query = kysely
      ...
      .$if(options.isFavorite != null, (qb) => qb.where('assets.isFavorite', '=', options.isFavorite as boolean))
      .$if(options.isOffline != null, (qb) => qb.where('assets.isOffline', '=', options.isOffline as boolean))
      .$if(options.isVisible != null, (qb) => qb.where('assets.isVisible', '=', options.isVisible as boolean))
      .$if(options.type != null, (qb) => qb.where('assets.type', '=', options.type as AssetType))
      .$if(options.isArchived != null, (qb) => qb.where('assets.isArchived', '=', options.isArchived as boolean))
      .$if(options.isEncoded != null, (qb) => qb.where('assets.encodedVideoPath', 'is not', null))
      .$if(options.isMotion != null, (qb) => qb.where('assets.livePhotoVideoId', 'is not', null))
      .$if(options.isNotInAlbum != null, (qb) =>
        qb
        ...
        qb.innerJoin(
          (eb: any) =>
            eb
              ...
              .where('asset_faces.personId', '=', sql<string>`any(array[${options.personIds}]::uuid[])`)
